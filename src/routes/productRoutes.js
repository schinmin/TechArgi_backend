...
const productImage = require('../middlewares/upload');
const admin = [protect, authorize('Admin')];
router.get('/', protect, controller.list);
router.post('/', [...admin, productImage, body('name').notEmpty(), body('category').isMongoId(), body('price').isFloat({ min: 0 }), body('cost').isFloat({ min: 0 }), body('sku').notEmpty(), validate], controller.create);
...
router.patch('/:id', ...admin, productImage, controller.update);
